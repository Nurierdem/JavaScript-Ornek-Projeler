...
function write(e) {
    ...
    if (element.classList.contains("number")) {
        ...
        const result = calculate(firstNumber, selectedOperator, afterNumber).toString();
        firstNumber = result;
        isWaitingANewValue = false;
...
function updateResultPanel(value) {
    const currentContent = resultEl.innerHTML;
    const newContent = currentContent + value;
    
    // Sadece sayısal değerler için uzunluk kontrolü yap
    const numericValue = parseFloat(newContent.replace(/[^0-9.]/g, ''));
    if (!isNaN(numericValue) && newContent.length >= 15) {
        resultEl.innerHTML = numericValue.toFixed(2);
    } else {
